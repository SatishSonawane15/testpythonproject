...
function addOptions(data, fileCard, i, is_option) {
    ...
    if(is_option){
        ...
        moveBtn.classList.add("btn","col-md-4","bg-light","rounded-0","m-0");
       // moveBtn.innerHTML = "<i class='fas fa-trash text-danger'></i>";
        moveBtn.innerHTML = "<i class='fas fa-cut text-danger'></i>";
        moveBtn.onclick = function(){
            Swal.fire({
              title: 'Are you sure?',
              text: "You won't be able to revert this move!",
              icon: 'warning',
              ...
              if (result.value) {
                ...
                moveFile({'user_id':data.user_id,'file_id':data.id, 'file_link':data.file_link.substring(1)}, fileCard);
              }
            });
        ...
        optionsCard.appendChild(moveBtn);
...
function showFolders(data,is_option) {
    ...
    for(i = 0; i < data.length; i++){
                ...
                if(is_option){
                    ...
                    //---MOVE FOLDER-------------------------------------------------------------
                    let moveBtn = document.createElement("button");
                    let moveBtnhandler = function(row_data){
                        return function () { moveFolder(row_data.user_id,row_data.folder_link,row_data.id,row); }
                    };
                    moveBtn.onclick = moveBtnhandler(data[i]);
                    moveBtn.classList.add("btn","btn-danger","m-2");
                    moveBtn.innerHTML = "<i class='fas fa-cut text-white'></i>";
                    optionCol.appendChild(moveBtn);
...
function moveFile(sendData,element) {
    $.ajax({
        url: "move",
        type: "post",
        data: sendData,
        ...
        success: function(data) {
             if(data.Status){
                ...
                alert("Not able to move! try again!");
            }
        }
    });
}
